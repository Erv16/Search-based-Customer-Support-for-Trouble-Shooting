...
public class ProductFour implements DSeaGateI{
	...
	private ArrayList<String> prodFourResponses = null;
	String temp ="";
	...
	public ProductFour(){
		...
		MyLogger.writeMessage("Reached constructor of Product Four",DebugLevel.CONSTRUCTOR);
		try{	
			FileProcessor fp = new FileProcessor(System.getProperty("user.dir")+"/src/troubleShootSearch/products/Repository4.txt");
			prodFourResponses = new ArrayList<String>();
				while((temp = fp.readLine()) != null){
					prodFourResponses.add(temp);
				} // end of while block
		} // end of try block
		catch(IOException ioe){
			System.err.println("Error: Unable to parse file Repository4.txt");
			ioe.printStackTrace();
			System.exit(1);
		} // end of catch block
	}
	 ...
	 * @param prodFourResponsesIn the data structure that needs to be set
	 */

	public void setProductContent(ArrayList<String> prodFourResponsesIn){
		prodFourResponses = prodFourResponsesIn;
	}
